...
class Fetch:
    ...
    def __init__(self, log_types):
        ...
        for line in self.log_list_response.iter_lines():
            parser = LogListParser()
            fragment = line.decode("utf-8")
            # We're interested in specific log files
            if not any(log_type in fragment for log_type in self.log_types):
                continue
            ...
            if local_log_file.is_file():
                self.logger.info("{} exists; skipping download".format(log_filepath))
                continue
            print("fetching {}".format(log_url))
            log_file_response = self.session.get(log_url)
            if log_file_response.status_code != 200:
                self.logger.error(
                    "Download error: HTTP {}".format(log_file_response.status_code)
                )
                continue
            if log_purepath.suffix == ".gz":
                self.logger.info("decompressing {}".format(log_url))
                content = gzip.decompress(log_file_response.content)
            else:
                content = log_file_response.content
            self.logger.info("saving {}".format(log_filepath))
